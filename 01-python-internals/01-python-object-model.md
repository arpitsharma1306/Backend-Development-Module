...
**Everything in Python is an object** — numbers, strings, functions, classes, modules, even `None`. And every object, no matter what it is, carries exactly three things:
...
A **variable name is not a box that holds a value** — it's a **label (reference) pointing at an object**. This one mental shift explains almost every "weird" Python behavior around copying, function arguments, and mutability.
...
<details>
<summary>📊 See it as a diagram (click to expand)</summary>

```
 Names (labels)              Object (lives on the heap)
 ─────────────               ──────────────────────────
   x  ────────┐
              ├──────────▶  [ list object: [1, 2, 3, 4] ]
   y  ────────┘              id: 140712834...
...
`x` and `y` are two different names pointing at **one** object. Mutating through `y` is visible through `x` because there was only ever one list.
...
* In **CPython** (the reference implementation you're almost certainly using), `id()` happens to return the object's **memory address**. That's an implementation detail, not a language guarantee — don't rely on the *value* itself, only on the fact that it's unique-per-object.
...
> **Rule of thumb:** use `is` only for singletons — `None`, `True`, `False`, and (carefully) `Enum` members. Everywhere else, use `==`.

```python
if x is None:      # ✅ idiomatic, fast, correct
    ...
if x == None:       # ⚠️ works but not idiomatic, can be fooled by __eq__ overrides
    ...
```

---

## 🏷️ `type()` — what kind of object
...
<sub>*A tuple is immutable as a *container* (you can't reassign its slots), but if it holds a mutable object, that inner object can still change. `t = ([1,2], 3); t[0].append(4)` works fine — `t` itself never changed identity.</sub>
...
```python
def add_item(item, bucket=[]):   # ⚠️ danger: default created ONCE at def time
    bucket.append(item)
    return bucket
...
`bucket=[]` is evaluated **once**, when the function is defined — not every call. Since lists are mutable, every call that doesn't pass `bucket` shares the *same* object.
...
> ⚠️ **Never rely on this behavior in real code.** It's a CPython implementation detail that can differ between versions and other interpreters (PyPy, Jython). It's a great interview/trivia topic, a terrible thing to depend on in production. Always compare values with `==`, not identity, unless you specifically mean "same object."
...
## 🛠️ Useful tools while studying this topic
...
<details>
<summary>🧠 Quick self-test — click to reveal answers</summary>
